...
// Fetches the live PAYE bands / statutory deduction rates (NSSF, Housing Levy, SHIF) / personal
// relief configured via Payroll Setup, converting backend shapes (minAmount/rate-as-percentage,
...
export const loadPayrollConfig = async (): Promise<Partial<PayrollConfig>> => {
  const [bands, statutory, settings] = await Promise.all([
  ...
  return {
    ...
    statutory: statutory.map((s: any) => ({
      ...
      category: s.category === "NSSF" ? "nssf" : s.category === "HOUSING_LEVY" ? "housing_levy" : s.category === "SHIF" ? "shif" : "other",
      value: s.value,
      maxAmount: s.maxAmount ?? null,
      minAmount: s.minAmount ?? null,
      thresholdAmount: s.thresholdAmount ?? null,
      active: s.active,
    })),
