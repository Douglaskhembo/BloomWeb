...
const AssignItemsModal = ({
  open, onOpenChange, title, description, items, selectedIds, onSave,
  multiple = true, searchPlaceholder = "Search...",
}: AssignItemsModalProps) => {
  const [query, setQuery] = useState("");
  const [picked, setPicked] = useState<string[]>(selectedIds);

  useEffect(() => {
    if (open) { setPicked(selectedIds); setQuery(""); }
  }, [open, selectedIds]);
  ...
  const allFilteredSelected = filtered.length > 0 && filtered.every(i => picked.includes(i.id));
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <Input placeholder={searchPlaceholder} value={query} onChange={e => setQuery(e.target.value)} className="pl-9" />
          </div>
          ...
          <div className="max-h-72 overflow-y-auto rounded-md border divide-y">
            {filtered.length === 0 ? (
              ...
              return (
                <label key={item.id} className="flex items-start gap-3 px-3 py-2.5 cursor-pointer hover:bg-muted/50 transition-colors">
        ...
        <DialogFooter>
          ...
          <Button onClick={() => { onSave(picked); onOpenChange(false); }}>
